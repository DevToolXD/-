...
6. **테마**: 상단의 "테마" 탭(또는 학생 사이드바 6번째 항목)에서 화면 전체의
   ...
   넓어졌고, 각 테마마다 색 미리보기와 선택 버튼이 있습니다. 고른 테마는
   기기에 저장되어 새로고침해도 유지됩니다.

   | 테마 | 설명 |
   |---|---|
   | 기본 | 애플처럼 담백한 유리 화면 |
   | 뽀로로 (테무산) | 하늘색 겨울 테마 + 내리는 눈 + 짝퉁 펭귄 마스코트 |
   | **독일** | 아래 참고 |

   **독일 테마**는 화면 전체를 하나의 무대로 씁니다. 전부 외부 이미지 없이
   인라인 SVG와 CSS로 그렸습니다.
   ...
   - 지평선의 금빛 노을과 비네트, 카드마다 금박 테두리 + 국기 실선,
     제목엔 금박이 흐르는 각인 효과
   ...
   세 테마 모두 본문·메뉴 글자의 명암비를 실제로 재서 WCAG 권장선(4.5:1)을
   넘는지 확인했습니다(측정값 4.7~5.8).
